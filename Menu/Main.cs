...
public class Main : MonoBehaviour
{
    ...
    {
        if (categoryName?.StartsWith("Console", StringComparison.OrdinalIgnoreCase) == true &&
            !HamburburData.IsLocalSuperAdmin)
            return;
    ...
    // Typewriter coroutine: reveal text while respecting rich text tags
    private IEnumerator TypewriterRoutine(Text target, string fullText, float delayPerChar)
    {
        if (target == null)
            yield break;
        ...
        for (int i = 0; i < fullText.Length; i++)
        {
            ...
            // If we hit the start of a rich tag, mark that we're inside a tag so we don't pause.
            if (c == '<')
            {
                inRichTag = true;
            }
            ...
            // If we've reached the end of a rich tag, stop treating characters as part of the tag.
            if (inRichTag && c == '>')
            {
                inRichTag = false;
                // Do not wait between characters inside tags; continue immediately.
                continue;
            }

            // Only wait for visible characters (not while inside a rich text tag).
            if (!inRichTag)
    ...
    public static void CreateMenu(bool playOpenAnimation = true)
    {
        ...
        // Start with empty text; coroutine will reveal characters
        titleText.text = string.Empty;

        // Store the full title string with page number and rich text
        string fullTitle = Constants.Name + " <color=grey>[</color><color=white>" + (pageNumber + 1) + "</color><color=grey>]</color>";

        // Ensure the menu GameObject has a Main component and start the typewriter coroutine
        Main mainComponent = menu.GetComponent<Main>() ?? menu.AddComponent<Main>();
        mainComponent.StartCoroutine(mainComponent.TypewriterRoutine(titleText, fullTitle, 0.03f));
    ...
    public static void Toggle(ButtonInfo target, bool? incrementDirection = null)
    {
        switch (target.mode)
        {
            ...
            case ButtonMode.Action:
            {
                ...
                {
                    NotifiLib.SendNotification(
                            "<color=grey>[</color><color=cyan>CONSOLE</color><color=grey>]</color> " +
                            target.buttonText + " - " + target.toolTip);
            ...
            case ButtonMode.Incremental:
            {
                ...
                NotifiLib.SendNotification(
                        "<color=grey>[</color><color=cyan>SETTING</color><color=grey>]</color> " +
                        target.buttonText + " - " + target.toolTip);
    ...
    private static void RefreshSearchResults()
    {
        ...
        searchResults = (from button in allButtons
                         where button.IsVisible
                         let displayText = button.GetDisplayText()
                         let nameMatch = displayText.IndexOf(searchQuery, StringComparison.OrdinalIgnoreCase) >= 0
                         let toolTipMatch = !string.IsNullOrEmpty(button.toolTip) &&
                                            button.toolTip.IndexOf(searchQuery, StringComparison.OrdinalIgnoreCase) >= 0
